...
// M4 added `archive` (plan §2.5) and the baseline ledger. M5 (this
// milestone) adds `guard` (plan §2.4) and wires it as a post-archive
// self-check. The remaining verb (init) lands in a later milestone (§8).
package main
...
func run(ctx context.Context, args []string) error {
	cmd := &cli.Command{
		...
		// TODO(M6): wire `init` here once it lands — see
